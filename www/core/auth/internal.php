...
class auth
{
	...
	function check_session ()
	{
		...
		# Check session...
		$data = kdb_select ('user', '*', '`id`=\''.$uid.'\' AND `login_key`=\''.md5($kid).'\' LIMIT 1');
		...
		# Update DB
		kdb_update_by_id('user', $data['id'], array('last_login'=>time(),'last_ip'=>ip2long($_SERVER['REMOTE_ADDR'])));
		# Update cookie
...
function get_admins ()
{
	$data = kdb_select ('user', array('id'), '`is_admin`=\'Y\' ORDER BY `id`');
...
function user_password ($login, $password, $is_md5 = false)
{
	...
	if ($u_data = kdb_select('user', array('id'), '`mail`=\''.kdb_escape($login).'\' AND `passwd`=\''.md5(md5($password)).'\' LIMIT 1'))
	{
		return $u_data[0]['id'];
	} else {
		return false;
