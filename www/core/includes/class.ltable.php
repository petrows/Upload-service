...
class ltable {
	...
	function add_no_items($text = false, $tags = true) {
		if (!$text && function_exists('lang'))
			$text = 'No items';
		if ($tags)
			$text = '<div style="text-align:center;padding:15px;">' . $text . '</div>';
	...
	function get_table() {
		...
		if ($this->class_table)
			$out .= ' class="' . $this->class_table . '"';
		if ($this->table_style)
			$out .= ' style="' . $this->table_style . '"';
		$out .= " >\n";
		...
		for ($x = 0; $x < count($this->table_data); $x++) {
			...
			for ($i = 0; $i < count($row); $i++) {
				...
				$row_out .= "\t<td";
				if (!@$row[$i]['align'])
					$row[$i]['align'] = $this->td_def_align;
				if (!@$row[$i]['valign'])
					$row[$i]['valign'] = $this->td_def_valign;
	...
	function _get_th() {
		if (!$this->table_th_data)
			return;

		$out = '<tr class="' . $this->class_row_th . '">' . "\n";
		for ($x = 0; $x < count($this->table_th_data); $x++) {
			$out .= '	<th';
			if (@$this->table_th_data[$x]['class'])
				$out .= ' class="' . $this->table_th_data[$x]['class'] . '"';
			if (@$this->table_th_data[$x]['width'])
				$out .= ' width="' . $this->table_th_data[$x]['width'] . '"';
			if (@$this->table_th_data[$x]['align'])
				$out .= ' align="' . $this->table_th_data[$x]['align'] . '"';
			$out .= '>';
			if (@$this->table_th_data[$x]['title'])
				$out .= $this->table_th_data[$x]['title'];
			$out .= "</th>\n";
		}
		$out .= "</tr>\n";
		return $out;
