...
/*
 * Throw exception on error or not (criticals - connection, etc)
 */
if (!defined('LDB_EXEPTION_ON_ERROR'))
	define('LDB_EXEPTION_ON_ERROR', true);
/*
 * Throw exception on query or not (errors in query)
 */
if (!defined('LDB_EXEPTION_ON_QUERY'))
	define('LDB_EXEPTION_ON_QUERY', true);
/*
...
class LdbSystemException extends Exception {
...
class LdbQueryException extends LdbSystemException {

	private $query;

	public function __construct($query, $sql_code, $sql_message) {
		$this->query = $query;
		parent::__construct($sql_message, $sql_code, null);
	}
...
class ldb {
	...
	function init($cfg_name = false) {
		...
		if (!isset($GLOBALS['ldb_config'][$cfg_name])) {
			...
			if (LDB_EXEPTION_ON_ERROR) {
				throw new LdbSystemException("No such config '$cfg_name'");
			} else {
		...
		if (!class_exists($driver_class, false)) {
			# Error - no such config record
			if (LDB_EXEPTION_ON_ERROR) {
				throw new LdbSystemException("No such driver '{$GLOBALS['ldb_config'][$cfg_name]['type']}'");
			} else {
		...
		if (!@$this->drivers[$cfg_name]) {
			...
			if (!$this->drivers[$cfg_name]->init()) {
				if (LDB_EXEPTION_ON_ERROR) {
					throw new LdbSystemException("Driver '{$GLOBALS['ldb_config'][$cfg_name]['type']}' error: {$this->drivers[$cfg_name]->error()}");
				} else {
	...
	function query($q) {
		...
		if (LDB_EXEPTION_ON_QUERY && $driver->error())
			throw new LdbQueryException($q, 0, $driver->error());
...
function ldb_log_html() {
	# Check - we have instance?
	if (!isset($GLOBALS[LDB_OBJECT_VAR]))
		return '<p>LDB is not active now.</p>';
...
class ldb_mysqli extends ldb_driver {
	...
	function query($q) {
		...
		$this->last_error = $this->mysqli->error;
		$this->last_error_code = $this->mysqli->errno;
...
class ldb_mysql extends ldb_driver {
	...
	function query($q) {
		...
		$this->last_error = mysql_error($this->mysql);
		$this->last_error_code = mysql_errno($this->mysql);
