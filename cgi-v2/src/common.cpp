...
vector<string> strExplode(string &s, const char *delm, size_t max) {
	...
	if (string::npos != pos1)
		ret.push_back(""); // Delimeter was at the end, add real empty line
...
string strLtrim(const string &sIn, const char *search) {
	...
	if (NULL == search) {
		...
		s.erase(s.begin(), std::find_if(s.begin(), s.end(), [&search](const char &c) {
					const char *z = search;
					while (*z) {
						if (*z == c)
							return false;
...
string strTrim(const string &sIn, const char *search) { return strLtrim(strRtrim(sIn, search), search); }
