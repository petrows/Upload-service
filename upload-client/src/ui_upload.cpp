...
ui_upload::ui_upload(QWidget *parent, u_upload *upl) : QDialog(parent), upl(upl), ui(new Ui::ui_upload) {
	ui->setupUi(this);
...
void ui_upload::reload() {
	// Parts setup
	if (this->upl->state == u_upload::Error) {
		QMessageBox::warning(this, tr("Upload error!"), this->upl->last_error);
		close();
		return;
	}
