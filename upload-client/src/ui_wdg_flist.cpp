...
ui_wdg_flist::ui_wdg_flist(QWidget *parent, u_upload *upl) : QTreeView(parent) {
	this->upl = upl;
...
ui_wdg_flist_control::ui_wdg_flist_control(QObject *parent) : QItemDelegate(parent) {}
...
QModelIndex ui_wdg_flist_mdl::parent(const QModelIndex & /*index*/) const { return QModelIndex(); }
int ui_wdg_flist_mdl::rowCount(const QModelIndex &parent) const {
	if (!parent.isValid())
		return this->upl->files.count();
	return 0;
}
int ui_wdg_flist_mdl::columnCount(const QModelIndex & /*parent*/) const { return 4; }
