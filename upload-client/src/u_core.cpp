...
void u_core_sleep::msleep(int ms) { QThread::msleep(ms); }

void __core_exit(int status = 0) { exit(status); }

u_core::u_core(QObject *parent) : QObject(parent) {
	this->_ui_main = NULL;
	this->_ui_init = NULL;
	this->tray = NULL;
	this->uploads_seed = 0;
...
void u_core::start_new_sid() {
	...
	for (int x = 0; x < u_list.count(); x++) {
		if (u_list.at(x).nodeName() != "upload")
			continue;
...
bool u_core::sid_ready() { return !this->sid.isEmpty(); }
