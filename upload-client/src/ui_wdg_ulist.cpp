...
ui_wdg_ulist::ui_wdg_ulist(QWidget *parent) :
	QTreeView(parent)
{
...
void ui_wdg_ulist::copy_links()
{
	...
	for (int x=0; x<ind.count(); x++)
	{
		...
		if (u_was.contains(upl)) continue;
		u_was.append(upl);
...
ui_wdg_ulist_control::ui_wdg_ulist_control(QObject *parent) :
...
QModelIndex ui_wdg_ulist_mdl::parent(const QModelIndex &/*index*/) const
{
	return QModelIndex();
}
...
int ui_wdg_ulist_mdl::columnCount(const QModelIndex &/*parent*/) const
{
	return 3;
}
