...
u_upload_file::u_upload_file(QObject *parent) : QObject(parent) {
...
void u_upload_file::_req_done() {
	...
	if (!api.parse_answer(data) || api.ans_dom.documentElement().firstChildElement("upload").attributes().namedItem("status").nodeValue() != "ok") {
		...
		qDebug() << "API error: " << this->last_error;
		emit this->state_changed(this->state);
		emit this->status_changed();
		this->parent->recount();
		core->reload_ui();
		return;
...
void u_upload_file::flist_progr_del() { this->progr_flist = NULL; }

QString u_upload_file::file_link() { return "http://" UPLOAD_DOMAIN "/c/d/" + this->parent->code + "/" + QString::number(this->file_n) + "/" + this->file_dh + "/" + QUrl::toPercentEncoding(this->file_name); }
...
u_upload::u_upload(QObject *parent) : QObject(parent) {
	this->state = Uploaded;
	this->id = 0;
	this->size_total = 0;
	this->size_upoaded = 0;
...
void u_upload::ulist_progr_del() { this->progr_ulist = NULL; }

QString u_upload::public_link() { return "http://" UPLOAD_DOMAIN "/f/" + this->code + "/" + this->ph + "/"; }
...
bool u_upload::has_files_ok() { return this->has_files_status(u_upload_file::Uploaded); }
bool u_upload::has_files_error() { return this->has_files_status(u_upload_file::Error); }
bool u_upload::has_files_uploading() { return this->has_files_status(u_upload_file::Uploading); }
...
int u_upload::files_count() { return this->files.size(); }
...
bool u_upload::start_file(QString path) {
	...
	connect(fl, SIGNAL(state_changed(u_upload_file::FileState)), SLOT(file_upload_status(u_upload_file::FileState)));
