...
u_update::u_update(QObject *parent) :
    QObject(parent)
{
...
void u_update::check_updates ()
{
	...
	for (int x=0; x<v_d.childNodes().count(); x++)
	{
		...
		html_desc += "<h3><span style='color:gray;'>"
				+ 		        QDateTime::fromTime_t(v_dd.attributes().namedItem("tms_publish").nodeValue().toInt()).toString("dd MMM yyyy")
				+ " - </span>" + v_dd.attributes().namedItem("version").nodeValue() + "</h3>";
